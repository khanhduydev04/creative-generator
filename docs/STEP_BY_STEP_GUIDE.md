...
> Hướng dẫn sử dụng tool tạo quảng cáo. Version: March 2026
>
...
1. Nhập email `@patigroup.com` + password → Click **"Sign In"**
...
## 2. Thiết lập Client & Brand

1. Click **"+ New Client"** trên header → nhập tên → **"Create"**
2. Chuyển client: click dropdown Client Selector → chọn client khác
3. Đổi tên / Xóa: click icon **"..."** cạnh client selector

![Header với client selector và actions](screenshots/02-client.png)

---

## 3. Thiết lập Brand Identity

Vào **Brand** trên navigation → điền các phần sau → click **"Save Brand Kit"**.
...
Vào **Create Ads** trên navigation. Layout 2 cột: trái (config) | phải (results).
...
Trong Create Ads → **Generation Mode** → chọn **"Competitor Reference"**.
...
Vào **Stealth** trên navigation. Flow 2 bước: Plan → Generate.
...
Vào **Admin** (icon shield) — chỉ CEO + Super Admin.

### Users

- Stats: Total, Active, Inactive, Admins
- Actions: Create User, Reset Password, Deactivate, Promote/Demote, Delete

### API Keys

Tab **Settings** → update Google API Key, KIE API Key, Google Console API Key.
Click edit → nhập key mới → **"Save"** (active ngay, không cần restart).
